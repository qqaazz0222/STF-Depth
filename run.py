...
def init_args():
    ...
    parser.add_argument('--datasets', type=list, default=["vp_test"], help='Dataset names for depth estimation')
...
def draw_segment_map(path, image, seg_map):
    ...
    segment_map_image = Image.fromarray(seg_map.astype(np.uint8)).resize(image.size)
...
def draw_panoptic_map(path, image, panoptic_map, segments_info):
    """Panoptic 분할 맵 시각화 함수"""
    color_palette = defaultdict(lambda: (np.random.randint(0, 255), np.random.randint(0, 255), np.random.randint(0, 255)))
    
    if hasattr(panoptic_map, 'cpu'):
        panoptic_map_np = panoptic_map.cpu().numpy()
    else:
        panoptic_map_np = panoptic_map

    segmented_image_np = np.zeros((panoptic_map_np.shape[0], panoptic_map_np.shape[1], 3), dtype=np.uint8)
...
def _visualize_depth_map_single(args):
    """깊이 맵 시각화 단일 프레임 처리 (multiprocessing용)"""
    idx, depth_map, depth_map_dir = args
    depth_map_path = os.path.join(depth_map_dir, f"{idx:08d}.png")
    draw_depth_map(depth_map_path, depth_map)


def _visualize_segment_map_single(args):
    """세그멘테이션 맵 시각화 단일 프레임 처리 (multiprocessing용)"""
    idx, image, segment_map, segment_map_dir = args
    segment_map_path = os.path.join(segment_map_dir, f"{idx:08d}.png")
    draw_segment_map(segment_map_path, image, segment_map)


def _visualize_segment_detail_map_single(args):
    """세그멘테이션 상세 맵 시각화 단일 프레임 처리 (multiprocessing용)"""
    idx, image, segment_detail_map, segments_info, segment_detail_map_dir = args
...
def visualize_results(working_dir, frame_list, depth_map_list, segment_map_list, segment_detail_map_list, segment_detail_info_list):
    ...
    args_list = [(idx, depth_map, depth_map_dir) for idx, depth_map in enumerate(depth_map_list)]
    with Pool(num_cores) as pool:
        list(tqdm(
            pool.imap(_visualize_depth_map_single, args_list),
            total=len(args_list),
            desc=" - Visualizing depth map"
        ))

    # Segment Map Visualization
    # Ensure tensors are converted to numpy arrays
    segment_map_numpy_list = [m.cpu().numpy() if hasattr(m, 'cpu') else m for m in segment_map_list]
    args_list = [(idx, image, segment_map, segment_map_dir) for idx, (image, segment_map) in enumerate(zip(frame_list, segment_map_numpy_list))]
    with Pool(num_cores) as pool:
        list(tqdm(
            pool.imap(_visualize_segment_map_single, args_list),
            total=len(args_list),
            desc=" - Visualizing segment map"
        ))
        
    # Segment Detail Map Visualization
    # Ensure tensors are converted to numpy arrays
    segment_detail_map_numpy_list = [m.cpu().numpy() if hasattr(m, 'cpu') else m for m in segment_detail_map_list]
    args_list = [(idx, image, segment_detail_map, segments_info, segment_detail_map_dir) 
                 for idx, (image, segment_detail_map, segments_info) in enumerate(zip(frame_list, segment_detail_map_numpy_list, segment_detail_info_list))]
    with Pool(num_cores) as pool:
        list(tqdm(
            pool.imap(_visualize_segment_detail_map_single, args_list),
            total=len(args_list),
            desc=" - Visualizing segment detail map"
        ))
